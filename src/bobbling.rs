...
impl qobject::TodoList {
    fn row_count(self: &TodoList, _parent: &QModelIndex) -> i32 {
    ...
    fn data(self: &TodoList, index: &QModelIndex, role: i32) -> QVariant {
        ...
        if let Some((done, todo)) = self.todos.get(index.row() as usize) {
            match role {
                TodoRoles::Todo => {
                    return todo.into();
                }
    ...
    fn role_names(self: &TodoList) -> QHash_i32_QByteArray {
    ...
    fn set_checked(mut self: Pin<&mut TodoList>, row: i32, checked: bool) {
    ...
    fn sort(mut self: Pin<&mut TodoList>) {
    ...
    fn add_todo(mut self: Pin<&mut TodoList>, todo: &QString) {
